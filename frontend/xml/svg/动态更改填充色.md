...
新版 chrome 浏览器已支持 SVG 2 的样式继承模型 允许外部 SVG 文件从引用的父级 <use> 元素那里继承 CSS 属性（例如你设置的 fill: iconColor）。

简单理解，Chrome 将 <use> 元素引用外部 SVG 的行为，处理得更像是一个“内部组件”，允许外部样式渗透进去。

而微软的 Edge 浏览器，目前遵循的是更早、更严格的 SVG 1.1 规范。

在 SVG 1.1 的规则下，存在一条明确的样式隔离原则：CSS 样式无法跨越文件边界 。当你引用外部的 demo.svg 文件时，它被当做一个独立的、受保护的文档。你在父页面写的任何 CSS 规则（无论是通过类名、ID 还是元素选择器）都无法作用到那个外部 SVG 文件的内部元素上。因此，你通过 :style="{ fill: iconColor }" 设置的样式就无法生效。在 edge 、火狐浏览器中更适合提前加载 svg 内容，在页面中通过 symbolId 引入使用并动态更改颜色。
